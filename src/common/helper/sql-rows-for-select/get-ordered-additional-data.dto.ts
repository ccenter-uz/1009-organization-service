    ...
    Prisma.sql`
      WITH
        AdditionalTranslations AS (
          ...
          FROM additional_translations at
          WHERE (${data.allLang} = TRUE OR at.language_code = ${data.langCode})
        ...
        AdditionalWarningTranslations AS (
          ...
          WHERE (${data.allLang} = TRUE OR awt.language_code = ${data.langCode})
        ...
        AdditionalMentionTranslations AS (
          ...
          WHERE (${data.allLang} = TRUE OR amt.language_code = ${data.langCode})
      ...
      SELECT
        ...
        (SELECT AdditionalContent FROM AdditionalContent WHERE additional_id = a.id) AS "AdditionalContent",
        (SELECT AdditionalTable FROM AdditionalTable WHERE additional_id = a.id) AS "AdditionalTable"
      FROM
        additional a
      ${whereClause}
      GROUP BY a.id
